const express = require('express');
const mongoose = require('mongoose');
const cors = require('cors');
const dotenv = require('dotenv');

dotenv.config();

const app = express();
const PORT = process.env.PORT || 5000;

// Middleware
app.use(cors());
app.use(express.json({ limit: '5mb' }));
app.use(express.urlencoded({ limit: '5mb', extended: true }));

// Metrics Middleware
const SystemMetric = require('./models/SystemMetric');
app.use(async (req, res, next) => {
  if (req.path.startsWith('/api')) {
    try {
      const today = new Date().toISOString().split('T')[0];
      const currentHour = new Date().getHours();
      
      let metric = await SystemMetric.findOne({ date: today });
      if (!metric) {
        metric = new SystemMetric({ date: today });
      }
      
      metric.apiCalls += 1;
      metric.hourlyApiCalls[currentHour] += 1;
      
      // Rough token estimation for /api/ai endpoints
      if (req.path.startsWith('/api/ai')) {
        metric.estimatedTokens += 500; // rough guess
      }

      // Mark the array as modified so Mongoose saves it
      metric.markModified('hourlyApiCalls');
      
      await metric.save();
    } catch (err) {
      console.error('Error updating SystemMetrics:', err);
    }
  }
  next();
});

// 測試 API
app.get('/api/health', (req, res) => {
  res.json({ status: 'ok', message: 'API is running perfectly!' });
});

// 根目錄路由
app.get('/', (req, res) => {
  res.send('Welcome to AI Learning System API Server');
});

// 路由設定
app.use('/api/auth', require('./routes/auth'));
app.use('/api/announcements', require('./routes/announcements'));
app.use('/api/questions', require('./routes/questions'));
app.use('/api/folders', require('./routes/folders'));
app.use('/api/exams', require('./routes/exams'));
app.use('/api/progress', require('./routes/progress'));
app.use('/api/analysis', require('./routes/analysis'));
app.use('/api/quiz', require('./routes/quiz'));
app.use('/api/ai', require('./routes/ai'));
app.use('/api/ai', require('./routes/ai-utils'));
app.use('/api/admin/knowledge', require('./routes/adminKnowledge'));
app.use('/api/admin/logs', require('./routes/adminLogs'));
app.use('/api/admin/analytics', require('./routes/adminAnalytics'));

const path = require('path');

// 資料庫連線 (等待您補上 MONGODB_URI)
if (process.env.MONGODB_URI) {
  mongoose.connect(process.env.MONGODB_URI)
    .then(() => console.log('✅ 成功連接至 MongoDB Atlas'))
    .catch((err) => console.error('❌ MongoDB 連線失敗:', err));
} else {
  console.log('⚠️ 尚未設定 MONGODB_URI，請在 .env 檔案中補上。');
}

// 全域錯誤捕捉 (攔截 Payload Too Large)
app.use((err, req, res, next) => {
  if (err.type === 'entity.too.large') {
    return res.status(413).json({
      success: false,
      message: '檔案或圖片過大！請確認大小不超過 5MB。'
    });
  }
  next(err);
});

// 啟動伺服器
app.listen(PORT, '127.0.0.1', () => {
  console.log(`🚀 伺服器運行於 http://127.0.0.1:${PORT}`);
});
